...
This is a case study from 'data with Danny' where I leverage MySQL 8.0 Workbench to handle business queries, demonstrating my skills in window functions, nested queries, and data transformation. 

The table creation section containing the data is shown at the end of the page as there are 1000 records.
To view the complete case study, please visit [this page](https://8weeksqlchallenge.com/case-study-3/).
...
```sql
CREATE VIEW subscriptions_with_change AS(
SELECT
...
SELECT * FROM subscriptions_with_change;
```
...
### Question: Based off the 8 sample customers provided in the sample from the subscriptions table, write a brief description about each customer’s onboarding journey.
...
| customer_id | plan_name      | new_plan_name  | price | new_price | start_date | date_change | days_before_change |
|-------------|----------------|----------------|-------|-----------|------------|-------------|--------------------|
| 1           | trial          | basic monthly  | 0.00  | 9.90      | 2020-08-01 | 2020-08-08  | 7                  |
| 2           | trial          | pro annual     | 0.00  | 199.00    | 2020-09-20 | 2020-09-27  | 7                  |
| 11          | trial          | churn          | 0.00  |           | 2020-11-19 | 2020-11-26  | 7                  |
| 13          | trial          | basic monthly  | 0.00  | 9.90      | 2020-12-15 | 2020-12-22  | 7                  |
| 13          | basic monthly  | pro monthly    | 9.90  | 19.90     | 2020-12-22 | 2021-03-29  | 97                 |
| 15          | trial          | pro monthly    | 0.00  | 19.90     | 2020-03-17 | 2020-03-24  | 7                  |
| 15          | pro monthly    | churn          | 19.90 |           | 2020-03-24 | 2020-04-29  | 36                 |
| 16          | trial          | basic monthly  | 0.00  | 9.90      | 2020-05-31 | 2020-06-07  | 7                  |
| 16          | basic monthly  | pro annual     | 9.90  | 199.00    | 2020-06-07 | 2020-10-21  | 136                |
| 18          | trial          | pro monthly    | 0.00  | 19.90     | 2020-07-06 | 2020-07-13  | 7                  |
| 19          | trial          | pro monthly    | 0.00  | 19.90     | 2020-06-22 | 2020-06-29  | 7                  |
...
**Assuming this is RIGHT after trial and not including change of plan after that e.g., not basic monthly to pro annual etc**
...
WHERE 
	plan_name = 'trial'
	AND new_plan_name != 'churn'
GROUP BY new_plan_name;
```

| percentage | num_cust | plan_after_trial |
|------------|----------|------------------|
|    60.1    |   546    |   basic monthly  |
|    4.1     |   37     |     pro annual   |
|    35.8    |   325    |    pro monthly   |
...
|------------|----------|----------------|
|    14.8    |   120    | basic monthly  |
|    32.9    |   267    | pro monthly    |
|    23.3    |   189    | pro annual     |
|    29.1    |   236    | churn          |
...
**Assuming this includes customer that has upgraded but then switched again to other plans/churns**

```sql
SELECT COUNT(DISTINCT customer_id) AS num_cust_annual_in_2020
FROM subscriptions_with_change
WHERE 
...
```sql
WITH days_til_annual_plan_table AS(
		WITH cust_annual_plan AS(
			SELECT
				customer_id,
				plan_id,
				start_date
		...
		SELECT
			customer_id,
			start_date,
			LEAD(start_date) OVER (PARTITION BY customer_id) AS annual_date,
			DATEDIFF(LEAD(start_date) OVER (PARTITION BY customer_id), start_date) AS days_til_annual_plan
...
WITH bin_tag_table AS(
		...
		SELECT
			customer_id, 
			days_til_annual_plan,
			CASE 
				WHEN days_til_annual_plan <= 30 AND days_til_annual_plan >= 0 THEN '0-30'
				WHEN days_til_annual_plan <= 60 AND days_til_annual_plan > 30 THEN '31-60'
				WHEN days_til_annual_plan <= 90 AND days_til_annual_plan > 60 THEN '61-90'
				WHEN days_til_annual_plan <= 120 AND days_til_annual_plan > 90 THEN '91-120'
				WHEN days_til_annual_plan <= 150 AND days_til_annual_plan > 120 THEN '121-150'
				WHEN days_til_annual_plan <= 180 AND days_til_annual_plan > 150 THEN '151-180'
				WHEN days_til_annual_plan <= 210 AND days_til_annual_plan > 180 THEN '181-210'
				WHEN days_til_annual_plan <= 240 AND days_til_annual_plan > 210 THEN '211-240'
				WHEN days_til_annual_plan <= 270 AND days_til_annual_plan > 240 THEN '241-270'
				WHEN days_til_annual_plan <= 300 AND days_til_annual_plan > 270 THEN '271-300'
				WHEN days_til_annual_plan <= 330 AND days_til_annual_plan > 300 THEN '301-330'
				ELSE '331-360'
				END AS bin_tag
		FROM
			days_til_annual_plan_table
...
GROUP BY
	bin_tag
ORDER BY
	CAST(SUBSTRING_INDEX(bin_tag, '-', 1) AS SIGNED);
```
| COUNT(customer_id) | bin_tag |
|---------------------|---------|
|         49          |  0-30   |
|         24          | 31-60   |
|         34          | 61-90   |
